...
const Check = (props) => {
	return <div className={props.checked ? 'checked' : 'unchecked'} style={props.style} />;
};
			...
			<ContextMenuTrigger id={'' + props.idkey}>
				<div className="node-ctn"
					onClick={(e) => {
						...
						props.toggleCheck ? props.toggleCheck() : console.log();
					}}
				>
					...
					>
						<Check
							...
							type="checkbox"
							style={{ marginLeft: props.iconClass ? props.iconClass.style.marginLeft : '0px' }}
						/>
						{props.iconClass !== '' ?
					
							<div className="iconos">
								<i className={props.iconClass.className} style= {{color: props.iconClass.style.color}}></i>
							</div>
							: ''}
					...
					{
						props.hasChildren ?
							<div className="icon-ctn" onClick={props.toggleEvent ? (e) => { e.stopPropagation();; props.toggleEvent() } : null}>
